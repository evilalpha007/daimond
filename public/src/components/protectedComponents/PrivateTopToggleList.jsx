...
export default function PrivateTopToggleList({ options, setoptions }) {
  ...
  return (
    <ul className="flex  items-center  margin_top_side overflow-scroll scrollbar-hidden ">
      {options.map((item, i) => {
        return (
          <li
            key={i}
            onClick={() => !item.active && toggleOptions(item)}
            className={`font-semibold bg-[#0289cd] inline-block whitespace-nowrap mt-[12px]  text-white items-center  p-2 px-4 border-r-[1px] border-[#0f2462] cursor-pointer ${
